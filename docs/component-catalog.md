...
| 右ペイン | 一覧の行から、画面を移動せずに詳細を右側に表示する。開いたまま一覧を操作できる | Bootstrap+自前 | `offcanvas offcanvas-end ds-offcanvas` > `ds-offcanvas-header` / `ds-offcanvas-body` / `ds-offcanvas-footer`、開く `bootstrap.Offcanvas.getOrCreateInstance(...).show()`(開いたまま別の行を押すと切り替わる)。Esc で閉じる処理は `app/static/app/offcanvas.js`、閉じる `data-bs-dismiss="offcanvas"` | [offcanvas.html](design-system/offcanvas.html) | [右ペイン](design-system.md#右ペイン) |
