...
class TestDepartmentIndexView:
    ...
    # 全部門閲覧を許可する権限セットを持つユーザーには会社を問わず一覧に表示されること
    ...
    # 特定の会社に絞った権限セットを持つユーザーには、その会社の部門のみ表示されること
...
class TestDepartmentShowView:
    ...
    # 部門閲覧の権限が無いユーザーがアクセスすると403が返ること
...
class TestDepartmentCreateView:
    ...
    # 会社を絞った作成権限を持つユーザーはGETでフォームを取得できること
    ...
    # 権限の対象範囲内の会社であれば作成でき詳細ページにリダイレクトされること
    ...
    # 管理者が無効なデータでPOSTするとフォームが再表示されること
    ...
    # 同じ会社に同名の部門が既にある場合、エラー付きでフォームが再表示されること
...
class TestDepartmentEditView:
    ...
    # 対象範囲内の部門であれば編集でき詳細ページにリダイレクトされること
    ...
    # 同じ会社の他部門と名前が重複する場合、エラー付きでフォームが再表示され更新されないこと
    def test_post_duplicate_name_redisplays_form_with_error(self, admin_client):
        company = Company.objects.create(name='サンプル株式会社')
...
class TestDepartmentDeleteView:
    ...
    # company_scoped_department_managerは削除を明示的にdenyしているため、403が返ること
