...
class TestManagementGroupModel:
    ...
    # is_admin=Falseなのに部門が未設定の場合はエラーになること(DB制約。Serviceの事前条件チェックが一次防衛)
    def test_department_required_when_not_admin(self):
        with pytest.raises(IntegrityError):
            ManagementGroup.objects.create(name='不正なグループ', permission_set_id=1)

    # is_admin=Trueなのに部門が設定されている場合はエラーになること(DB制約。Serviceの事前条件チェックが一次防衛)
    ...
    # is_admin=Falseなのに権限セット番号が未設定の場合はエラーになること(DB制約。Serviceの事前条件チェックが一次防衛)
    ...
    # is_admin=Trueなのに権限セット番号が設定されている場合はエラーになること(DB制約。Serviceの事前条件チェックが一次防衛)
    ...
    # 名前が重複する場合はエラーになること(DB制約。Serviceの事前条件チェックが一次防衛)
