...
作業中は`tmp/`に置き、完了時にPRへ転記して`tmp/`のファイルを削除する。

- **必ず、テストを書く前に人へ提示して確認を取る。** テストを書いた後に見せると、人は「テストと合っているか」しか見なくなり、仕様そのものの誤りが検出されない
- **作業中のドラフトは`tmp/spec-<機能名>.md`に置く** — 手順1で作成し、確認を受けて修正する。手順2〜5の間、テストの変換元として参照し続ける。会話だけに置くと、実装が進むうちに箇条書きを見失い、結局その時点の実装都合でテストを書くことになる
