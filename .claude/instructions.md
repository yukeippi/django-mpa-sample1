...
`tmp/spec/`に置き、完了時にPRへ転記する。`tmp/spec/`のファイルは完了後も削除せずに残す。

- **必ず、テストを書く前に人へ提示して確認を取る。** テストを書いた後に見せると、人は「テストと合っているか」しか見なくなり、仕様そのものの誤りが検出されない
- **作業中のドラフトは`tmp/spec/<作成日時>-<機能名>.md`に置く**(例: `tmp/spec/202609230909-task-status.md`。作成日時は`YYYYMMDDHHMM`形式で、ファイル名の先頭に付ける)。`tmp/`には他の作業ファイルも置かれるため、仕様は`tmp/spec/`にまとめて他と区別し、日時を先頭に付けて作成順に並ぶようにする — 手順1で作成し、確認を受けて修正する。手順2〜5の間、テストの変換元として参照し続ける。会話だけに置くと、実装が進むうちに箇条書きを見失い、結局その時点の実装都合でテストを書くことになる
- **`tmp/`はGitで追跡しない**(`.gitignore`に`tmp/`を追加する)。追跡される場所に仕様を置くと「保守される」期待が生まれるが、実際には保守されず、次の実装者(特にAI)が古い仕様を現在の正として読む
- **`tmp/`が無いプロジェクトでは、作成してよいか人に確認する。** 断られた場合はファイルを作らず、箇条書きを会話に出して確認を取るだけにとどめる(その場合もPRへの転記は行う)
...
- **`tmp/spec/`に残したファイルは、作成日時の時点の仕様の記録であり、現在の仕様の正本ではない。** 完了後に更新しない。後から読むときは、ファイル名の作成日時を確認し、過去の経緯を辿る目的でのみ参照する。コード・テストと食い違っている場合はコードが正
- **設計ドキュメントを作るワークフローを使う場合** — その中に「仕様」節として置き、`tmp/`は使わない。アーキテクチャやテストの置き場所を書く節とは別に立てる。設計(どう作るか)と仕様(どんな条件で何が起きるか)は別物であり、設計から導いたテストは実装構造の写しになる
