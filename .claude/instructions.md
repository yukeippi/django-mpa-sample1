...
設計判断の記録(ADR)は、コピー先のプロジェクトで `docs/adr/` に作る(ADR Rules参照)。このテンプレート自体には同梱しない。
...
- **「却下した案」を推測で書かない**: 実際に検討して捨てた案だけを書く。一般論としてもっともらしい理由を補わない。代替案と却下理由が示されていない場合は、その節を空のまま残して確認する
...
以下は書式の例示。番号・タイトル・日付は説明のための仮のもので、コピー先の実際のADRとは対応しない。ここでは「`save()` から `full_clean()` を呼ぶ」という決定(ADR-0004)をコピー先が後から覆した、という想定にしている。
