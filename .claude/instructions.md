...
作業中は`tmp/spec/`に置き、完了時にPRへ転記して`tmp/`のファイルを削除する。
...
- **完了時にPRの説明欄へ転記し、`tmp/`のファイルを削除する**(PRを作らない場合はコミットメッセージの本文に書く)。テストの妥当性を検証するには、テストとは独立した基準が必要になる。テストが仕様の唯一の記録だと、仕様の取り違えが実装とテストの両方に一貫して現れ、レビューで検出できない(「期待値を本番コードから導出しない」と同じ理由。Test Specification Rules参照)
- **転記後の削除を忘れない。** `tmp/`に残り続けるファイルは、それ自体が「更新されない古い仕様書」になる
