...
- **設計ドキュメントを作るワークフローを使う場合** — その中に「仕様」節として置く。アーキテクチャやテストの置き場所を書く節とは別に立てる。設計(どう作るか)と仕様(どんな条件で何が起きるか)は別物であり、設計から導いたテストは実装構造の写しになる
- **使わない場合** — 会話に出して確認を取るだけでよい。ファイルには残さない。変換後の仕様の正本はテストコードであり、同じ仕様を別の文書にも置くと二重管理になり、更新されない側が腐る(ADR Rulesと同じ考え方)
